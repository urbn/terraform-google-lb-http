...
resource "google_compute_backend_service" "default" {
  ...
  security_policy                 = length(split(",", element(var.backend_params, count.index))) > 6 && element(split(",", element(var.backend_params, count.index)), 6) != "" ? element(split(",", element(var.backend_params, count.index)), 6) : var.security_policy
